...
$id = $_GET['id']; 
$sql = "SELECT Category_Name from productcategory 
        ...
        if($result->num_rows >0){
           ...
           echo'<div><h1>'.$row['Category_Name'].'</h1></div>';
        }
?>
<div id="dropdownSort">
    <label>Sort By:</label>
...
if (isset($_GET['sortproduct'])) {
    ...
    switch ($sortOption) {
        ...
        case 'bestselling':
            ...
            JOIN productcategory ON products.CategoryID = productcategory.Category_id
            WHERE productcategory.Category_id = $id
            GROUP BY products.Product_id, productcategory.Category_Name
            ORDER BY total_quantity DESC";
            break;
        case 'pricelowtohigh':
            $sql = " SELECT * FROM  products  JOIN productcategory ON products. CategoryID = productcategory.Category_id 
        WHERE productcategory.Category_id =$id ORDER BY price Asc";
            break;
        case 'pricehightolow':
            $sql = " SELECT * FROM  products  JOIN productcategory ON products. CategoryID = productcategory.Category_id 
        WHERE productcategory.Category_id =$id ORDER BY price DESC";
            break;
        case 'alphabet':
            $sql = "SELECT * FROM  products  JOIN productcategory ON products. CategoryID = productcategory.Category_id 
        WHERE productcategory.Category_id =$id ORDER BY Product_Name ASC";
            break;
        case 'reversealphabet':
            $sql = " SELECT * FROM  products  JOIN productcategory ON products. CategoryID = productcategory.Category_id 
        WHERE productcategory.Category_id =$id ORDER BY Product_Name DESC";
            break;
    }
}
